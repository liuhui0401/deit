...
import math
from reference_dataloader.util.zipreader import is_zip_path, ZipReader
    ...
    def __init__(
        ...
        if '22k' in root:
            ...
            if cluster == '1400':
                ...
                annotation_root = '/mnt/lustre/wangwenhai.vendor/IN22K/'
        else:
            # Imagenet
...
class ParserCephImage(Parser):
    def __init__(
        ...
        if on_memory:
            ...
            print("{}{}{}{}{}{}".format(self.local_rank, self.local_size, self.rank, self.world_size, self.num_replicas, self.num_parts))
    ...
    def load_onto_memory(self):
        ...
        for index in trange(len(self.samples)):
            ...
            # mmcv.imresize(img, (384,384))
            # img_bytes = mmcv.imencode(img)
    ...
    def __getitem__(self, index):
        ...
        # filepath = '/mnt/lustre/share/images/val/ILSVRC2012_val_00002991.JPEG'
        if cluster == '1424':
            filepath = filepath.replace('image22k/', 'imagenet22k/')

        try:
            if self.on_memory:
                if filepath in self.holder:
                    img_bytes = self.holder[filepath]
                else:
                    img_bytes = self.file_client.get(filepath)
                    self.holder[filepath] = img_bytes
            else:
                # pass
                img_bytes = self.file_client.get(filepath)
            img = mmcv.imfrombytes(img_bytes)[:, :, ::-1]
