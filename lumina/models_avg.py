...
class Attention(nn.Module):
    ...
    def apply_rotary_emb(
        ...
        with torch.cuda.amp.autocast(enabled=False):
            cls_q = xq[:, 0, :, :].unsqueeze(1)
            cls_k = xk[:, 0, :, :].unsqueeze(1)
            xq_ = torch.view_as_complex(xq[:, 1:, :, :].float().reshape(*xq[:, 1:, :, :].shape[:-1], -1, 2))
            xk_ = torch.view_as_complex(xk[:, 1:, :, :].float().reshape(*xk[:, 1:, :, :].shape[:-1], -1, 2))
            ...
            xq_out = torch.cat([cls_q, xq_out], dim=1)
            xk_out = torch.cat([cls_k, xk_out], dim=1)
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if adaln_input is not None:
            ...
            h = x + gate_msa.unsqueeze(1) * self.drop_path(self.attention(
                modulate(self.attention_norm(x), shift_msa, scale_msa),
                freqs_cis,
            ))
            out = h + gate_mlp.unsqueeze(1) * self.drop_path(self.feed_forward(
                modulate(self.ffn_norm(h), shift_mlp, scale_mlp),
            ))

        else:
            h = x + self.gate_sa.tanh() * self.attention_norm2(self.drop_path(self.attention(self.attention_norm(x), freqs_cis,)))
            out = h + self.gate_ffn.tanh() * self.ffn_norm2(self.drop_path(self.feed_forward(self.ffn_norm(h))))
...
class DiT_Llama(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList([
            TransformerBlock(layer_id, dim, n_heads, n_kv_heads, multiple_of,
                             ffn_dim_multiplier, norm_eps, qk_norm, drop_path=drop_path_rate)
        ...
        self.freqs_cis = DiT_Llama.precompute_freqs_cis(24, 384)
    ...
    def forward_features(self, x):
        ...
        freqs_cis = self.freqs_cis[:H, :W].flatten(0,1).unsqueeze(0)
        freqs_cis = freqs_cis.to(x.device)
        # no position embedding for cls token
        x = torch.cat((cls_token, x), dim=1)
