...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if adaln_input is not None:
            ...
            h = x + gate_msa.unsqueeze(1).tanh() * self.attention_norm2(self.drop_path(self.attention(
                modulate(self.attention_norm(x), scale_msa),
                freqs_cis,
            )))
            out = h + gate_mlp.unsqueeze(1).tanh() * self.ffn_norm2(self.drop_path(self.feed_forward(
...
class DiT_Llama(nn.Module):
    ...
    def __init__(
        ...
        self.t_embedder = ParallelTimestepEmbedder(dim)
    ...
    def forward_features(self, x, t=None):
        ...
        if t is None:
            # for inference, set noise to zero
            t = torch.ones_like(x.shape[0])
    ...
    def forward(self, x, t=None):
        if t is None:
            t = torch.ones(x.shape[0]).to(x.device)
